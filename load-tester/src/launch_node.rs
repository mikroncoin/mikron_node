...
pub fn launch_node(
    ...
    match fs::create_dir(&data_dir) {
        ...
        r => r.chain_err(|| "failed to create mikron_node data directory")?,
    }
    ...
    let config = json!({
        ...
        "node": {
            ...
            "preconfigured_representatives": [
                "mik_37qjexk5phhd9fin11z68dsmsmxirhm6isptm8pdb39kp6z5w8e1534tigqk"
            ],
            "inactive_supply": "0",
    ...
    let child = Command::new(mikron_node)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn mikron_node")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call mikron_node RPC"))
            .map(|_| ()),
    ) as _
}
