...
void rai::block_span_analyzer::analyze (boost::filesystem::path data_path)
{
	...
	std::vector<long> account_span_account_counts;
	std::vector<long> account_span_blocks_counts;
	long cnt = 0;
	...
	// computing medians
	rai::blockchain_analyzer::printMedianStats ("Account counts", account_span_account_counts);
	rai::blockchain_analyzer::printMedianStats ("Block counts", account_span_blocks_counts);
}
