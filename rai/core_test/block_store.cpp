...
TEST (block_store, one_block)
{
	...
	rai::block_store store (init, rai::unique_path ());
	ASSERT_TRUE (!init);
	rai::state_block block1 (0, 0, 0, 1, 0, 0, rai::keypair ().prv, 0, 0);
	rai::transaction transaction (store.environment, nullptr, true);
...
TEST (block_store, one_account)
{
	...
	rai::transaction transaction (store.environment, nullptr, true);
	store.account_put (transaction, account, { hash, account, hash, 0, 42, 100, 200 });
	auto begin (store.latest_begin (transaction));
	auto end (store.latest_end ());
	ASSERT_NE (end, begin);
...
TEST (block_store, two_block)
{
	...
	rai::state_block block2 (2, 0, 0, 1, 0, 0, rai::keypair ().prv, 0, 0);
...
TEST (block_store, two_account)
{
	...
	rai::transaction transaction (store.environment, nullptr, true);
	store.account_put (transaction, account1, { hash1, account1, hash1, 0, 42, 100, 300 });
	store.account_put (transaction, account2, { hash2, account2, hash2, 0, 84, 200, 400 });
	auto begin (store.latest_begin (transaction));
	auto end (store.latest_end ());
	ASSERT_NE (end, begin);
...
TEST (block_store, latest_find)
{
	...
	rai::block_hash hash2 (4);
	rai::transaction transaction (store.environment, nullptr, true);
	store.account_put (transaction, account1, { hash1, account1, hash1, 0, 100, 0, 300 });
	store.account_put (transaction, account2, { hash2, account2, hash2, 0, 200, 0, 400 });
...
TEST (block_store, sequence_increment)
{
	...
	rai::keypair key1;
	rai::keypair key2;
	auto block1 (std::make_shared<rai::state_block> (0, 0, 0, 1, 0, 0, rai::keypair ().prv, 0, 0));
	rai::transaction transaction (store.environment, nullptr, true);
...
TEST (block_store, upgrade_v4_v5)
{
	...
	auto path (rai::unique_path ());
	{
		...
		rai::keypair key0;
		rai::state_block block0 = ::bs_create_send_state_block_helper (info.head, key0.pub, rai::genesis_amount - rai::Gxrb_ratio, rai::test_genesis_key);
...
TEST (block_store, upgrade_v9_v10)
{
	...
	rai::block_hash hash (0);
	{
		...
		for (auto i (1); i < 32; ++i) // Making 31 send blocks (+ 1 open = 32 total) (store.block_info_max)
		{
			...
			rai::state_block block0 = ::bs_create_send_state_block_helper (hash, key0.pub, balance, rai::test_genesis_key);
			ASSERT_EQ (rai::process_result::progress, ledger.process (transaction, block0).code);
			hash = block0.hash ();
		}
