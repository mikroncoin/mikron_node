...
echo "$DOCKER_PASSWORD" | docker login -u catenocrypt --password-stdin

# We push this just so it can be a cache next time
if [ "$TRAVIS_BRANCH" = "master" ]; then
    "$scripts"/custom-timeout.sh 30 docker push mikroncoin/mikron-ci
fi
...
for network in live beta; do
    ...
    docker_image_name="mikroncoin/mikron${network_tag_suffix}"
